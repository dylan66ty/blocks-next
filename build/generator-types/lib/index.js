...
async function readMarkdown(options) {
  const mdPaths = await (0, fast_glob_1.default)(
    (0, utils_1.normalizePath)(`${options.path}/**/*.md`)
  )
  const data = mdPaths
    ...
    .map((path) => {
      ...
      const componentName = (0, utils_1.getComponentName)(
        docPath.substring(docPath.lastIndexOf('/') + 1) || ''
      )
      const fileContent = (0, fs_extra_1.readFileSync)(path, 'utf-8')
      return (0, formatter_1.formatter)(
        (0, parser_1.mdParser)(fileContent),
        componentName,
        kebabComponentName,
        options.tagPrefix
      )
...
async function parseAndWrite(options) {
  ...
  ;(0, fs_extra_1.outputFileSync)(
    (0, path_1.join)(options.outputDir, 'tags.json'),
    JSON.stringify(veturTags, null, 2)
  )
  ;(0, fs_extra_1.outputFileSync)(
    (0, path_1.join)(options.outputDir, 'attributes.json'),
    JSON.stringify(veturAttributes, null, 2)
  )
  ;(0, fs_extra_1.outputFileSync)(
    (0, path_1.join)(options.outputDir, 'web-types.json'),
