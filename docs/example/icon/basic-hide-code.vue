...
<template>
  <div class="icon-wrapper">
    <div v-for="(group, key) in iconGroupData" :key="key" class="icon-group">
      <div class="icon-group-name">{{ group.label }}</div>
      <div class="icon-group-content">
        <div
          v-for="item in transIcons(group.filenames)"
          :key="item.n"
          class="icon-item"
          @click="handleCopy(item.n)"
        >
          <component :is="components[item.c]" class="icon"></component>
