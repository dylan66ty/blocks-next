    ...
    <Trigger v-model:popupVisible="states.popupVisible" position="bl" trigger="click" :unmount-on-close="false"
      ...
      <template #default>
        <div :class="[`${ns}__trigger`]">
          ...
          <div v-if="multiple && multipleTags.length" :class="[`${ns}__multiple`]" ref="multipleTagsRef">
            <div :class="[`${ns}__tag`]" v-for="(tag, index) in multipleTags" :key="`${tag.key}-${index}`">
...
export default defineComponent({
  ...
  setup(props, { slots, emit }) {
    ...
    watch(pathValueToNodeKeys, (nodeKeys) => {
      nextTick(() => {
    ...
    watch(() => states.popupVisible, (value) => {
      if (value) {
